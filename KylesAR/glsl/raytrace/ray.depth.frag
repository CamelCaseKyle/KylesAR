...
vec4 light = vec4(vec3(0., 10., 0.), 1.0);
//returns dist
...
const int mtSph = 2;
vec4 sphere = vec4(vec3(0., 8.0, 0.), 8.0);
//returns dist
...
vec4 sdfMarch( vec3 ro, vec3 rd, float tmax ) {
    ...
    //if we hit anything
    if (c.x < minC) col = map_rgba(pos, rd, c); //col = vec4(vec3(t), 1.0);
    //final color
    return col;
}
...
vec3 plasma_rgb( in vec3 pos, in float density ) {
    return normalize(pos.xzy + 1.0)*density*density;
}
...
void main() {
	...
	//fix cam depth
	float camDepth = texture2D(depthFrame, uv * 0.5 + 0.5).r * 245.0 + 50.0;
	if (camDepth < 50.1)
		camDepth = 999.8;
	...
	if (mode == 0) {
		...
		color = densityMarch(camLoc, rayDir, vec2(55.0, camDepth));

	//sdf march
	} else if (mode == 1) {
		
		//update the light over time
		light.x = cos(timeoffset)*11.0;
	    light.z = sin(timeoffset)*11.0;

		color = sdfMarch(camLoc, rayDir, camDepth);
	}
}
