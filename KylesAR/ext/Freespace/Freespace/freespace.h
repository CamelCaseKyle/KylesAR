...
#ifdef FREESPACE_EXPORTS
#define FREESPACE_API extern "C" __declspec(dllexport)
#else
#define FREESPACE_API extern "C"
#endif
...
class FreespaceIMU : public i_PositionTracker {
public:
	int enable_accel = 0,
		enable_accelNoGrav = 0,
		enable_angPos = 0,
	...
	PositionObject po = PositionObject("FreespaceIMU");
	glm::vec3 eulerAngles;
	int rc;

	// create an instance
	FreespaceIMU(int _enable_accel = 0, int _enable_accelNoGrav = 1, int _enable_angPos = 0, int _enable_angVel = 1, int _enable_compass = 0, int _enable_incline = 0, int _enable_mag = 0, int _enable_temp = 0);
	...
	int getMotionFromInputThread(struct InputLoopState* state, struct freespace_MotionEngineOutput* meOut);
...
FREESPACE_API i_PositionTracker* __stdcall allocate() {
	i_PositionTracker* ret = (i_PositionTracker*)(new FreespaceIMU());
	return ret;
}

FREESPACE_API void __stdcall destroy(i_PositionTracker* inst) {
	delete inst;
}
